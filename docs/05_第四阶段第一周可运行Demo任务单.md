...
> 状态：completed（C006/C007 已通过，冻结基线 `b51e955`）
...
- [x] P01 至 P08 连续走通至少两遍。
- [x] 第二遍档案与最近评估能够恢复。
- [x] 10 题题量、三维度和评分字段正确。
- [x] 业务源码和运行数据中不存在 `balanceIndex`。
- [x] 报告页有非医学诊断、非正式量表说明。
- [x] 五类课程卡齐全，多肉为默认推荐。
- [x] 页面不直接导入 Mock，不散落 Storage 调用。
- [x] 未新增真实登录、云开发、后台、社区或 P09 以后业务。
- [x] 两侧均提供完整交接信息，`main` 工作区干净可运行。

以上门槛现已全部通过，第四阶段正式完成。P09–P11 核心体验后半程等待项目负责人另行发布任务后再启动。
...
- A010、A011、A012、A030、A031 已完成并合入。
- B020、B021、B022、B023 已完成并合入。
- C003 P05→P06 联调通过。
- C006 P01→P08 两遍真实链路通过。
- C007 视觉、数据、状态恢复和重复提交交叉验收通过。

关键缺陷闭环：

- Mock 重复登录不再覆盖已经保存的自然档案。
- P07 可在重新加载后恢复最近评估与推荐课程。
- 微信小程序公共按钮和卡片事件已同步处理，不再出现 `method "1,0,0"`。
- P06 返回 P05 后答案锁定；查看报告复用原 `submissionId` 和 `createdAt`，不会重复保存。
- 从 P04 主动开始新评估时仍会清空本轮状态，并生成新的报告。
...
- 微信开发者工具游客模式提示。
- Node ES Module 类型提示。
- Sass legacy API 弃用提示。
- 开发者工具偶发外部 CDN `shadow-grey.png` 网络提示；不属于仓库本地资源。

以上提示不影响 P01→P08 演示链。第五阶段已于 2026-08-20 通过 `docs/06_第五阶段核心体验后半程任务单.md` 正式发布。
